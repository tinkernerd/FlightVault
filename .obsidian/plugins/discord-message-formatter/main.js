...
function formatMessageDate(date, settings) {
...
function textRunFactory(elem) {
  ...
  switch (elem.nodeName) {
    ...
    default: {
      ...
      } else if (/timestamp/.test(elem.className) && ((_a = elem == null ? void 0 : elem.firstElementChild) == null ? void 0 : _a.nodeName) == "TIME") {
        const timestamp = DateTime.fromISO((_b = elem == null ? void 0 : elem.firstElementChild) == null ? void 0 : _b.getAttribute("datetime"));
        if (timestamp) {
...
// src/messages/ReplyMessage.ts
var MAX_CHARS_BEFORE_SHORTEN = 70;
...
// src/messages/MultiMessage.ts
var MultiMessage = class {
  constructor(messageLi) {
    try {
  ...
  constructMessageHeader(messageLi) {
    var _a, _b;
    ...
    const avatarDiv = messageLi.querySelector("img[class^='avatar']");
  ...
  constructMessageContent(messageLi) {
  ...
  /** Gets elements to be consumed by utils/textRunFactory() */
  getMessageTextElems(messageLi) {
    var _a;
    return (_a = messageLi.querySelector("div[class^='contents'] > div[id^='message-content']")) == null ? void 0 : _a.children;
  }
  toMarkdown(settings) {
    const markdownArray = [];
    if (this.header) {
...
// src/messages/SingleMessage.ts
var SingleMessage = class extends MultiMessage {
...
// src/messages/SystemMessage.ts
var SystemMessage = class {
  constructor(messageLi) {
    var _a;
    const messageTextElems = (_a = messageLi.querySelector("div[class^='content_']")) == null ? void 0 : _a.childNodes;
    if (!messageTextElems) {
    ...
    if (!(timeElem == null ? void 0 : timeElem.dateTime)) {
...
var Conversation = class {
  ...
  createMessages(DOM) {
    ...
    for (const message of messageElems) {
      try {
        if (((_a = message.firstElementChild) == null ? void 0 : _a.className.includes("systemMessage")) || ((_b = message.firstElementChild) == null ? void 0 : _b.className.includes("system_message"))) {
          discordMessages.push(new SystemMessage(message));
        } else if ((_c = message.firstElementChild) == null ? void 0 : _c.innerHTML) {
...
var SettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerElement).setName("Date format").setDesc("The format the date/time of the message should be displayed in. (See www.foragoodstrftime.com)").addText((textField) => {
      textField.setValue(this.plugin.settings.dateFormat).onChange(async (value) => {
        this.plugin.settings.dateFormat = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerElement).setName("Copy (edited) tag").setDesc("How to copy the *(edited)* tag at the end of an edited message.").addDropdown((dropdown) => {
      dropdown.addOption("off", "Don't copy").addOption("text", "Plain text").addOption("tag", "Text with editing date on hover").setValue(this.plugin.settings.showEdited).onChange(async (value) => {
        this.plugin.settings.showEdited = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerElement).setName("Copy reply").setDesc("How to copy a message that's being replied to.").addDropdown((dropdown) => {
      dropdown.addOption("off", "Don't copy").addOption("shortened", "Shorten reply").addOption("full", "Full reply").setValue(this.plugin.settings.showReplies).onChange(async (value) => {
        this.plugin.settings.showReplies = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerElement).setName("Copy system messages").setDesc('Copy messages like "User started call", "User joined server", ...').addToggle((toggle) => {
      toggle.setValue(this.plugin.settings.showSystemMessages).onChange(async (value) => {
...
var DiscordFormatter = class extends import_obsidian2.Plugin {
  ...
  pasteMessage(event) {
    ...
    if (event instanceof ClipboardEvent) {
      rawHTML = (_a = event.clipboardData) == null ? void 0 : _a.getData("text/html");
    } else if (event instanceof DragEvent) {
      rawHTML = (_b = event.dataTransfer) == null ? void 0 : _b.getData("text/html");
    }
    const view = this.app.workspace.getActiveViewOfType(import_obsidian2.MarkdownView);
    if (!(rawHTML && (view == null ? void 0 : view.editor))) {
      return;
    }
    const conversation = Conversation.fromRawHTML(rawHTML);
